...
pub enum Inst {
    ...
    /// Match the start of a line without advancing.
    StartLine,
    /// Match the end of a line without advancing.
    EndLine,
    /// Match the start of the text without advancing.
    StartText,
    /// Match the end of the text without advancing.
    EndText,
    /// Match a non-space character immediately following a space character.
    WordBoundary,
...
impl Inst {
    ...
    pub fn matches(&self, c1: Char, c2: Char) -> bool {
        match *self {
            Inst::StartLine => c1.is_none() || c1 == '\n',
            Inst::EndLine => c2.is_none() || c2 == '\n',
            Inst::StartText => c1.is_none(),
            Inst::EndText => c2.is_none(),
            Inst::WordBoundary => c1.is_word_char() != c2.is_word_char(),
            Inst::NotWordBoundary => c1.is_word_char() == c2.is_word_char(),
            _ => unreachable!(),
        }
    }
}
...
/// The implementor is responsible for tracking the program counter,
/// if necessary.
pub trait Dispatch {
    ...
    /// A match has occurred.
    fn has_matched(&mut self) -> Self::Out;

    /// Save the current location in the input into the given capture location.
    fn save(&mut self, x: InstIdx, i: usize) -> Self::Out;

    /// Advance the input by a single character
    fn advance(&mut self) -> Self::Out;

    /// Jump to the instruction given.
    fn jump(&mut self, x: InstIdx) -> Self::Out;

    /// Abandon the current thread of execution.
    fn fail(&mut self) -> Self::Out;

    /// Match either instruction, preferring the first.
    fn split(&mut self, x: InstIdx, y: InstIdx) -> Self::Out;
}

#[inline(always)]
pub fn dispatch<T: Dispatch>(insts: &Vec<Inst>, engine: &mut T, at: InputAt, pc: usize) -> T::Out {
